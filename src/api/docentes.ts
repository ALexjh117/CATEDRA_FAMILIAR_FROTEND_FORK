...
export interface NotaCursoDocenteBackend {
...
export interface NotaDetalleBackend {
...
export interface EstudianteConNotasBackend {
  ...
  notas: NotaDetalleBackend[];

}



export interface NotasEstudianteDetalleBackend {
...
export interface RankingCursoBackend {
...
function normalizeWrappedData<T>(body: any, fallback: T): T {

  if (body && typeof body === 'object') {

    if ('data' in body) return (body.data ?? fallback) as T;

    if ('success' in body && 'data' in body) return (body.data ?? fallback) as T;
...
function normalizeNotaDetalle(item: any): NotaDetalleBackend {

  return {

    id: Number(item?.id ?? 0),

    nota: Number(item?.nota ?? item?.valor ?? item?.puntaje ?? 0),

    materia_nombre: item?.materia_nombre ?? item?.materiaNombre ?? item?.materia?.nombre ?? item?.asignatura ?? item?.area,

    titulo: item?.titulo ?? item?.nombre ?? item?.actividad,

    descripcion: item?.descripcion ?? item?.detalle,

    nota_cualitativa: item?.nota_cualitativa ?? item?.notaCualitativa ?? item?.concepto,

    retroalimentacion: item?.retroalimentacion ?? item?.feedback,

    calificado_en: item?.calificado_en ?? item?.calificadoEn ?? item?.fechaCalificacion,
...
export async function listarCursosNotasDocente() {

  const parseCursos = (body: any): NotaCursoDocenteBackend[] => {

    const data = normalizeWrappedData<any[]>(body, []);
    ...
    return Array.isArray(data)
      ...
      ? data.map((item: any) => ({
          ...
          nombre: item?.nombre ?? item?.nombreCurso ?? `Curso #${item?.id ?? ''}`,

          grado: item?.grado ?? item?.gradoNombre,

          total_estudiantes: Number(item?.total_estudiantes ?? item?.totalEstudiantes ?? 0),
  ...
  try {
    ...
    const response = await httpService.get<any>('/docentes/cursos');

    return parseCursos(response.data);

  } catch {

    const cursos = await listarCursos();

    return (Array.isArray(cursos) ? cursos : []).map((curso) => ({
...
export async function listarEstudiantesConNotas(params: { periodoId: number; cursoId?: number }) {

  const response = await httpService.get<any>('/notas/estudiantes', {
  ...
  const data = normalizeWrappedData<any[]>(response.data, []);
  ...
  return Array.isArray(data)
    ...
    ? data.map((item: any) => ({
        ...
        numero_documento: item?.numero_documento ?? item?.numeroDocumento,

        curso_nombre: item?.curso_nombre ?? item?.cursoNombre ?? item?.curso?.nombre,

        promedio: Number(item?.promedio ?? 0),

        total_asignaturas: Number(item?.total_asignaturas ?? item?.totalAsignaturas ?? 0),

        aprobadas: Number(item?.aprobadas ?? 0),

        reprobadas: Number(item?.reprobadas ?? 0),

        notas: Array.isArray(item?.notas) ? item.notas.map(normalizeNotaDetalle) : [],
...
export async function getNotasEstudiante(estudianteId: number, periodoId: number) {

  const response = await httpService.get<any>(`/notas/estudiante/${estudianteId}`, {

    periodo_id: periodoId,

  });

  const data = normalizeWrappedData<any>(response.data, null);
  ...
  return {
    ...
    estudiante: {
      ...
      id: Number(data?.estudiante?.id ?? estudianteId),

      nombres: data?.estudiante?.nombres ?? data?.estudiante?.nombre ?? '',

      apellidos: data?.estudiante?.apellidos ?? '',

      curso: data?.estudiante?.curso ?? data?.estudiante?.curso_nombre ?? data?.estudiante?.cursoNombre,

    },

    promedio: Number(data?.promedio ?? 0),

    total_asignaturas: Number(data?.total_asignaturas ?? data?.totalAsignaturas ?? 0),

    aprobadas: Number(data?.aprobadas ?? 0),

    reprobadas: Number(data?.reprobadas ?? 0),

    notas: Array.isArray(data?.notas) ? data.notas.map(normalizeNotaDetalle) : [],

  } as NotasEstudianteDetalleBackend;

}



export async function getRankingCurso(cursoId: number, periodoId: number) {

  const response = await httpService.get<any>(`/notas/curso/${cursoId}`, {
  ...
  return {
    ...
    curso: {
      ...
      id: Number(data?.curso?.id ?? cursoId),

      nombre: data?.curso?.nombre ?? `Curso #${cursoId}`,

    },

    total_estudiantes: Number(data?.total_estudiantes ?? data?.totalEstudiantes ?? 0),

    promedio_curso: Number(data?.promedio_curso ?? data?.promedioCurso ?? 0),

    estudiantes: Array.isArray(data?.estudiantes)

      ? data.estudiantes.map((item: any) => ({
...
export async function listarEstudiantesDocente(params?: { page?: number; limit?: number; q?: string }) {
  try {
    const response = await httpService.get<any>('/estudiantes/mis-estudiantes', params);
    ...
    // Fallback a estructuras alternativas
    ...
    console.error('Error en listarEstudiantesDocente:', error);
    return {
      data: [],
      total: 0,
      cursos: []
...
export async function listarAcudientesDocente(params?: { page?: number; limit?: number; q?: string }) {
  try {
    const response = await httpService.get<any>('/acudientes/mis-acudientes', params);
    ...
    console.error('Error en listarAcudientesDocente:', error);
    return {
      data: [],
      total: 0,
      cursos: []
...
export async function listarBancoTareasDocenteInstitucion(institucionIdOverride?: number) {
  // Obtener institucionId desde la sesión del usuario o desde el parámetro
  const user = getSessionUser();
  let institucionId = institucionIdOverride || user?.institucionId || user?.institucion_id;
  
  // Si todavía no hay institucionId, intentar obtenerlo de localStorage
  if (!institucionId) {
    try {
      const rawSession = typeof localStorage !== 'undefined' ? localStorage.getItem('session') : null;
      const parsedSession = rawSession ? JSON.parse(rawSession) : null;
      institucionId = parsedSession?.user?.institucionId || parsedSession?.user?.institucion_id;
    } catch {}
  }
  
  if (!institucionId) {
    // Si no hay institucionId, devolver respuesta vacía en lugar de error
    return {
      docente: undefined,
  ...
  try {
    ...
    // Si hay error en la llamada, devolver respuesta vacía
    return {
      docente: undefined,
      institucion: undefined,
      totalTareas: 0,
...
function resolveInstitucionIdPeriodos(explicitValue?: number) {

  const explicitParsed = Number(explicitValue);

  if (Number.isFinite(explicitParsed) && explicitParsed > 0) return explicitParsed;
  ...
  try {
    ...
    const candidate =
      ...
      sessionUser?.institucionId ??

      sessionUser?.institucion_id ??

      parsedSession?.context?.institucionId ??

      parsedSession?.institucionId ??

      parsedSession?.context?.institucion_id ??

      parsedUser?.institucionId ??

      parsedUser?.institucion_id;
  ...
  // En lugar de lanzar error, devolver undefined para que la función maneje el caso
  return undefined;

}

function parsePeriodos(body: any): PeriodoBackend[] {

  // Manejar la estructura de respuesta: {success: true, data: [...]}
  let list = [];
  
  if (body?.success === true && Array.isArray(body?.data)) {
    list = body.data;
  } else {
    // Estructuras alternativas para compatibilidad
    list =
      Array.isArray(body?.data?.data?.periodos) ? body.data.data.periodos :
      Array.isArray(body?.data?.periodos) ? body.data.periodos :
      Array.isArray(body?.data?.data) ? body.data.data :
      Array.isArray(body?.data) ? body.data :
      Array.isArray(body?.periodos) ? body.periodos :
  ...
  return list.map((item: any) => ({
    ...
    institucionId: item?.institucion_id ?? item?.institucionId,
    estaActivo: item?.esta_activo ?? item?.estaActivo ?? item?.activo ?? false,
  }));

}

export async function listarPeriodos(params?: { institucionid?: number; solo_activos?: boolean; page?: number; limit?: number; activo?: boolean }) {

  const institucionId = resolveInstitucionIdPeriodos(params?.institucionid ?? params?.institucionId);

  // Si no hay institucionId, devolver array vacío en lugar de error
  if (!institucionId) {
    return [];
  }
  ...
  if (params?.solo_activos !== undefined) {
    query.solo_activos = params.solo_activos;
  } else if (params?.activo !== undefined) {
    query.solo_activos = params.activo;
  }
  ...
  try {
    const response = await httpService.get<any>('/periodos', query);
    
    if (response.data?.success === false) {
      throw new Error(response.data?.message || 'Error obteniendo períodos');
    }
    
    return parsePeriodos(response.data);
  } catch (err: any) {
    if (err.message?.includes('institucionid es requerido')) {
      throw new Error('El ID de institución es requerido');
    }
    throw err;
  }
}
